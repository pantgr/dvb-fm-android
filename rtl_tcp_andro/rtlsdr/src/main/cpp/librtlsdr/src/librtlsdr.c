...
/* ===== Android patch (dvb_android 2026-06-11) =====================
 * rtlsdr_open2: άνοιγμα μέσω file descriptor από το Android USB Host
 * API (libusb_get_device2/libusb_open2 = patched vendored libusb του
 * rtl_tcp_andro). Σώμα = πιστό αντίγραφο του rtlsdr_open() πιο πάνω,
 * με ΜΟΝΗ αλλαγή το discovery/open κομμάτι. Κρατήστε το συγχρονισμένο
 * με το rtlsdr_open σε μελλοντικά updates της rtl-sdr-blog. */
...
int rtlsdr_open2(rtlsdr_dev_t **out_dev, int fd, const char *devicePath)
{
	...
	/* Android: το device έρχεται έτοιμο από το USB Host API ως fd+path */
	...
	if (r < 0) {
		fprintf(stderr, "usb_open2 error %d\n", r);
		goto err;
	}
